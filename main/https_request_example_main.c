...
#define WEB_SERVER "www.on-request-example-vvjuxrz3lq-uc.a.run.app"
#define WEB_PORT "443"
#define WEB_URL "https://on-request-example-vvjuxrz3lq-uc.a.run.app"

#define SERVER_URL_MAX_SZ 256

static const char *TAG1 = "wifi_example";
...
static void https_get_request(esp_tls_cfg_t cfg, const char *WEB_SERVER_URL, const char *REQUEST)
{
    ...
    if (!tls) {
        ESP_LOGE(TAG1, "Failed to allocate esp_tls handle!");
        goto exit;
    }

    if (esp_tls_conn_http_new_sync(WEB_SERVER_URL, &cfg, tls) == 1) {
        ESP_LOGI(TAG1, "Connection established...");
    } else {
        ESP_LOGE(TAG1, "Connection failed...");
        ...
        if (ret == ESP_OK) {
            ESP_LOGE(TAG1, "TLS error = -0x%x, TLS flags = -0x%x", esp_tls_code, esp_tls_flags);
        }
        goto cleanup;
    }
    ...
    ESP_LOGI(TAG1, "%d request size", strlen(REQUEST));
    do {
        ret = esp_tls_conn_write(tls,
                                 REQUEST + written_bytes,
                                 strlen(REQUEST) - written_bytes);
        if (ret >= 0) {
            ESP_LOGI(TAG1, "%d bytes written", ret);
            written_bytes += ret;
        } else if (ret != ESP_TLS_ERR_SSL_WANT_READ  && ret != ESP_TLS_ERR_SSL_WANT_WRITE) {
            ESP_LOGE(TAG1, "esp_tls_conn_write  returned: [0x%02X](%s)", ret, esp_err_to_name(ret));
            goto cleanup;
        }
    } while (written_bytes < strlen(REQUEST));

    ESP_LOGI(TAG1, "Reading HTTP response...");
    do {
        len = sizeof(buf) - 1;
        ...
        if (ret == ESP_TLS_ERR_SSL_WANT_WRITE  || ret == ESP_TLS_ERR_SSL_WANT_READ) {
            ...
            ESP_LOGE(TAG1, "esp_tls_conn_read  returned [-0x%02X](%s)", -ret, esp_err_to_name(ret));
            break;
        } else if (ret == 0) {
            ESP_LOGI(TAG1, "connection closed");
            break;
        }

        len = ret;
        ESP_LOGD(TAG1, "%d bytes read", len);
...
static void https_get_request2(esp_tls_cfg_t cfg, const char *WEB_SERVER_URL, const char *REQUEST, int size)
{
    ...
    size_t written_bytes = 0;
    ESP_LOGI(TAG1, "%d request size", size);
    do {
        ret = esp_tls_conn_write(tls,
                                 REQUEST + written_bytes,
                                 size - written_bytes);
...
{
    ESP_LOGI(TAG1, "https_request using crt bundle");
    esp_tls_cfg_t cfg = {
    ...
    int request_size = strlen(HOWSMYSSL_REQUEST) + image.size;
    char request[request_size];
    // memset(request,0,request_size);
    strcpy(request,HOWSMYSSL_REQUEST);
    memcpy(request + strlen(HOWSMYSSL_REQUEST),image.buf,image.size);
    ESP_LOGI(TAG1, "https_request using crt bundle");
    // int request_size = strlen(HOWSMYSSL_REQUEST);
...
{
    ESP_LOGI(TAG1, "https_request using cacert_buf");
    esp_tls_cfg_t cfg = {
...
// #if CONFIG_EXAMPLE_USING_ESP_TLS_MBEDTLS
    ...
    ESP_LOGI(TAG1, "https_request using server supported ciphersuites");
    esp_tls_cfg_t cfg = {
        .cacert_buf = (const unsigned char *) server_root_cert_pem_start,
        .cacert_bytes = server_root_cert_pem_end - server_root_cert_pem_start,
    ...
    ESP_LOGI(TAG1, "https_request using server unsupported ciphersuites");
...
static void https_get_request_using_global_ca_store(void)
{
    ...
    ESP_LOGI(TAG1, "https_request using global ca_store");
    esp_ret = esp_tls_set_global_ca_store(server_root_cert_pem_start, server_root_cert_pem_end - server_root_cert_pem_start);
    if (esp_ret != ESP_OK) {
        ESP_LOGE(TAG1, "Error in setting the global ca store: [%02X] (%s),could not complete the https_request using global_ca_store", esp_ret, esp_err_to_name(esp_ret));
        return;
    }
    esp_tls_cfg_t cfg = {
...
{
    ESP_LOGI(TAG1, "https_request to local server");
    esp_tls_cfg_t cfg = {
...
{
    ESP_LOGI(TAG1, "https_request using saved client session");
    esp_tls_cfg_t cfg = {
...
{
    ESP_LOGI(TAG1, "Start https_request example");
    ...
    if (strcmp(CONFIG_EXAMPLE_LOCAL_SERVER_URL, "FROM_STDIN") == 0) {
        ...
        ESP_LOGE(TAG1, "Configuration mismatch: invalid url for local server");
        abort();
    }
...
#endif
    ESP_LOGI(TAG1, "Minimum free heap size: %" PRIu32 " bytes", esp_get_minimum_free_heap_size());
    // https_get_request_using_cacert_buf();
    // https_get_request_using_global_ca_store();
    // https_get_request_using_specified_ciphersuites();
    ESP_LOGI(TAG1, "Finish https_request example");
    vTaskDelete(NULL);
}
...
//         ESP_LOGI(TAG1, "Updating time from NVS");
...
//     xTaskCreate(&https_request_task, "https_get_task", 18192, NULL, 5, NULL);
// }
